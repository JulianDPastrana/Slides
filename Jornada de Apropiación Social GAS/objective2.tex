...
\begin{frame}{Loss Function}
	...
	Each component is defined as the product of a \emph{unit cost} and its \emph{associated flow}:
	
%	\begin{columns}[T,onlytextwidth] 
%		\column{0.48\textwidth}
%		\begin{itemize}
%			\item Injection at node $w$: \emph{$C_{\text{iny}}^w$} × \emph{$f_{\text{iny}}^w$}
%			\item Transport through pipeline $p$: \emph{$C_{\text{trans}}^p$} × \emph{$f_{\text{trans}}^p$}
%		\end{itemize}
%		
%		\column{0.48\textwidth}
%		\begin{itemize}
%			\item Transport through compressor $c$: \emph{$C_{\text{trans}}^c$} × \emph{$f_{\text{trans}}^c$}
%			\item Unmet demand for user $u$ at node $i$: \emph{$C_{\text{rest}}^{u,i}$} × \emph{$f_{\text{rest}}^{u,i}$}
%		\end{itemize}
%	\end{columns}
\end{frame}
