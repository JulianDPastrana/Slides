...
\begin{frame}{Natural Gas Transportation Network}
		\begin{columns}[c,onlytextwidth] 
		
		\column{0.6\textwidth}
		\centering
		\includegraphics[height=\textheight]{colombia_network}
		
		\column{0.4\textwidth}
		The natural gas system is composed of nodes, pipelines, and compressors that must satisfy certain restrictions imposed by physical laws or operational standards.
	\end{columns}
\end{frame}
...
\begin{frame}{Natural Gas Transportation Network}
	...
	\centering
	\includegraphics[width=\textwidth]{colombia_network}
\end{frame}
...
\begin{frame}{Nonlinear Constraints and Weymouth Equation}
	...
	\[
	\text{sign}(f_{\text{trans}}^p)\,(f_{\text{trans}}^p)^2 = K_p \,(p_i^2 - p_j^2)
	\]
	
	To overcome this challenge, we employed Mathematical Programs with Complementarity Constraints (\emph{MPCC}). This approach replaces the disjunctive nature of the sign function with complementarity conditions, allowing continuous variables to replicate the behavior of binary decisions without resorting to mixed-integer formulations.
\end{frame}
