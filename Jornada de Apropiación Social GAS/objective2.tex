...
\begin{frame}{Loss Function}
	...
	\begin{columns}[T,onlytextwidth]
		...
		\begin{itemize}
			\item Injection at node $w$: \emph{C_{\text{iny}}^w} (cost) \emph{f_{\text{iny}}^w} (flow)
			\item Pipeline $p$: \emph{C_{\text{trans}}^p} (cost) \emph{f_{\text{trans}}^p} (flow)
		\end{itemize}
		\column{0.48\textwidth}
		\begin{itemize}
			\item Compressor $c$: \emph{C_{\text{trans}}^c} (cost) \emph{f_{\text{trans}}^c} (flow)
			\item Unmet demand at node $i$ for user $u$: \emph{C_{\text{rest}}^{u,i}} (cost) \emph{f_{\text{rest}}^{u,i}} (flow)
		\end{itemize}
	\end{columns}
\end{frame}
