...
\begin{frame}{Loss Function}
	...
	\begin{columns}[T,onlytextwidth]
		...
		\begin{itemize}
			\item Injection at node $w$: \mathhl{C_{\text{iny}}^w} (cost) $\times$ \mathhl{f_{\text{iny}}^w} (flow)
			\item Pipeline $p$: \mathhl{C_{\text{trans}}^p} (cost) $\times$ \mathhl{f_{\text{trans}}^p} (flow)
		\end{itemize}
		\column{0.48\textwidth}
		\begin{itemize}
			\item Compressor $c$: \mathhl{C_{\text{trans}}^c} (cost) $\times$ \mathhl{f_{\text{trans}}^c} (flow)
			\item Unmet demand at node $i$ for user $u$: \mathhl{C_{\text{rest}}^{u,i}} (cost) $\times$ \mathhl{f_{\text{rest}}^{u,i}} (flow)
		\end{itemize}
	\end{columns}
\end{frame}
