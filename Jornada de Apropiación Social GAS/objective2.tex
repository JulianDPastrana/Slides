...
\begin{frame}{Loss Function}
	...
	\begin{columns}[T,onlytextwidth] 
		...
		\begin{itemize}
			...
			\item Transport through pipeline $p$: \emph{$C_{\text{trans}}^p$ (cost)} $\times$ \emph{$f_{\text{trans}}^p$ (flow)}
		\end{itemize}
		
		\column{0.48\textwidth}
		\begin{itemize}
			\item Transport through compressor $c$: \emph{$C_{\text{trans}}^c$ (cost)} $\times$ \emph{$f_{\text{trans}}^c$ (flow)}
