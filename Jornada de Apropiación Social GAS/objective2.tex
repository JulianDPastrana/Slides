...
\begin{frame}{Nonlinear Constraints and Weymouth Equation}
	The model contains nonlinear constraints such as positivity of flows, operational limits, and capacity bounds. A major difficulty arises with the Weymouth equation for pipelines, which relates pressures at the ends of a pipe with the transported gas flow and introduces both nonconvexity and discontinuity:
	
	\[
	\text{sgn}(f_{\text{trans}}^p)\,(f_{\text{trans}}^p)^2 = K_p \,(p_i^2 - p_j^2)
	\]
	
	To overcome this challenge, we employed Mathematical Programs with Complementarity Constraints (MPCC). This approach replaces the disjunctive nature of the sign function with complementarity conditions, allowing continuous variables to replicate the behavior of binary decisions without resorting to mixed-integer formulations. The resulting model is computationally more efficient and was solved using the IPOPT solver, which exploits primal-dual interior-point methods to handle nonlinear optimization effectively.
\end{frame}
