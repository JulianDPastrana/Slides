...
\begin{frame}{The LMC Model}
	...
	We call the above as Linear Model of Coregionalization (LMC) \cite{8187598}.
...
\begin{frame}{Variational Optimization}
	...
	We introduce induced vectors \( \mathbf{u} \in \mathbb{R}^{MQ} \) with elements \( g_q(\mathbf{z}_m) \) at	\( M \ll N \) inducing points \( \{\mathbf{z}_m\}_{m=1}^M \) to reduce the complexity \emph{\(\mathcal{O}(N^3)\) to \(\mathcal{O}(NM^2)\)}, and approximating the joint posterior \cite{pmlr-v38-hensman15}
