...
\begin{frame}{Likelihood Model}
	...
	\begin{block}{}
		Each element of \( \boldsymbol{\theta}(\boldsymbol{x}) \), denoted as \( \theta_{j}(\boldsymbol{x}) \), could be restricted to some subset of \( \mathbb{R} \). To handle that, we model \( \theta_{j}(\boldsymbol{x}) = h_{j}(f_{j}(\boldsymbol{x})) \) as a transformation of an unrestricted latent variable \( f_{j}(\boldsymbol{x}) \) via a link function \( h_{j} \).
	\end{block}
\end{frame}
