\begin{tikzpicture}[
	...
	% Encoder, 1st layer
	\node[layer] (add1) at (1.25,3.83) {Add \& Norm};
	\node[layer] (attn1) at (1.25,2.78) {Masked \vspace{-0.05cm} \linebreak Multi-Head \vspace{-0.05cm} \linebreak Attention};
	\draw[] (attn1) -- (add1);
	% Encoder 2nd sub-layer
	\node[layer] (add4) at (1.25,6.38) {Add \& Norm};
	\node[layer] (ff1) at (1.25,5.53) {Feed \vspace{-0.05cm} \linebreak Forward};
	\draw[] (ff1) -- (add4);
	...
	\node[input, above=1.5em of add4] (z) {$\mathbf{f}$};
	\draw[arrow] (add4) -- (z);
