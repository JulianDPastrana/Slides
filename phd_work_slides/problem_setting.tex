...
\begin{frame}[t]{Problem Setting}
	...
	\begin{block}{}
		Let $\{\boldsymbol{x}^{(i)}, y^(i)\}_{i=1}^{N}$ be the train dataset, with $\boldsymbol{x}^{(i)}\in\mathcal{X}$ and $y \in \{0, 1, \cdots, 6\}$. $\mathcal{X}$ is the space of vector sequences with length $T = 256$, that is
		\[
		x^{(i)}
		=\bigl[\boldsymbol{x}^{(i)}_{1},\dots,\boldsymbol{x}^{(i)}_{t_{n_i}}\bigr],
