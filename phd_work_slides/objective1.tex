...
\begin{frame}{Transformer and Attention Mechanism}
	\begin{columns}[c] % [c] = vertically center
		...
		\begin{column}{0.48\textwidth}
			\centering
			\scalebox{0.5}{\input{figures/multihead_attention.tex}}
			\begin{itemize}
				\item $N_t$: Number of transformer layers
				\item $\boldsymbol{P}$: Positional embedding
				\item $\boldsymbol{Q} \in \mathbb{R}^{N \times T}$, $\boldsymbol{K} \in \mathbb{R}^{N \times T}$, $\boldsymbol{V} \in \mathbb{R}^{N \times d_v}$
			\end{itemize}
		\end{column}
